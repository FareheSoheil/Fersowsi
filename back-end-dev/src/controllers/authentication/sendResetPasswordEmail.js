  ...
  <mj-body background-color="#ffffff">
    ...
    <mj-section background-color="#ffffff">
      <mj-column>
        ...
        <mj-text font-size="20px" color="#000000" font-family="helvetica">Password Reset</mj-text>

        <mj-divider border-color="#353e5c"></mj-divider>

        <mj-text font-size="15px" color="#000000" font-family="helvetica">Someone asked us to send you a password reset link. If it was you, please press the button below within 24 hours.
        </mj-text>
        <mj-button background-color="#80ff80" color="white" align="left" font-family="helvetica" href="${link}"> Reset Password </mj-button>
        ...
        <mj-text font-size="15px" color="#000000" font-family="helvetica">If you didn't request password reset, just ignore this email.
        </mj-text>
...
const sendResetPasswordEmail = async (req, res) => {
  ...
  // Input validation
  if (!email) {
    resError(res, 500, titles.RESET_PASSWORD_EMAIL_ERROR, errors.EMAIL_IS_NOT_PROVIDED);
    return;
  }
  ...
  if (err) {
    resError(res, 500, titles.RESET_PASSWORD, errors.RESET_PASSWORD_LINK_NOT_SEND);
  }
  if (!user) {
  ...
  sendMail(
    ...
    htmlOutput(`${config.frontEndServer}/resetPassword/${user.resetPasswordToken}`).html,
  );

  resSuccess(
    res, 200, titles.RESET_PASSWORD, successes.RESET_PASSWORD_LINK_SENT,
  );
};
