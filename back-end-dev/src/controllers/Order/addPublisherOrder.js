...
import {  CustomerOrder, PublisherOrder } from '../../models';
import {
  validateId,
  validateName,
  validateFloatNumber,
...
const addPublisherOrder = async (req, res) => {
  const { 
      newUserId,
      newActionUserId,
  ...
  if (req.user) {
    ...
    res.status(200).json({message: 'publisher order is inserted'});
    return;
  }
  else {
...
export default addPublisherOrder;
