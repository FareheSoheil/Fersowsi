...
const setProductPeriodPrice = async (req, res) => {
  const { 
  ...
  if (req.user) {
    ...
    [err, productPeriodPrice] = await to(
      ProductPeriodPrice.findOne({where: {id:productPeriodPriceIdSanitized}
      }),
    );
    if (err) {
      resError(res, 500, titles.FETCH_STATE_ERROR, errors.DATABASE_ERROR);
    ...
    ProductPeriodPrice.productId = newproductIdSanitized;
    ProductPeriodPrice.productPeriodId = newProductPeriodIdSanitized;
    ProductPeriodPrice.PublisherPrice = newPublisherPriceSanitized;
    ProductPeriodPrice.CustomerPrice = newCustomerPriceSanitized;
    ...
    res.status(200).json({message: 'product period price updated'});
    return;
  }
  else {
...
export default setProductPeriodPrice;
